...
import { ReservationProps, createReservation, updateReservation } from "../../../../actions/reservations";

const statusOptions = [
  { label: "Aktif",              value: "ACTIVE" },
  { label: "Sözleşmeye Dönüştü", value: "CONVERTED" },
  { label: "İptal",              value: "CANCELLED" },
  { label: "Süresi Doldu",       value: "EXPIRED" },
  { label: "Kapora İade",        value: "REFUNDED" },
];
...
type Props = {
  listings: Listing[]; agents: Agent[]; clients: Client[];
...
export default function ReservationForm({ listings, agents, clients, agencyId, editingId, initialData }: Props) {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<any>({
    defaultValues: {
      ...
      reservedUntil: fmt(initialData?.reservedUntil),
  ...
  async function onSubmit(data: any) {
    ...
    if (!data.reservedUntil) { toast.error("Rezervasyon bitiş tarihi gerekli."); return; }
    ...
    try {
      setLoading(true);
      if (isEditing) { await updateReservation(editingId!, payload); toast.success("Rezervasyon güncellendi!"); }
      else { await createReservation(payload); toast.success("Rezervasyon oluşturuldu!"); reset(); }
      router.push("/estate/dashboard/reservations");
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <h2 className="text-xl sm:text-2xl font-extrabold tracking-tight text-black">
        {isEditing ? "Rezervasyonu Düzenle" : "Yeni Rezervasyon"}
      </h2>
      ...
      <fieldset className="border rounded-lg p-4 space-y-4">
        <legend className="text-sm font-semibold px-1 text-black">Rezervasyon Detayı</legend>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <TextInput register={register} errors={errors} label="Kapora Tutarı" name="depositAmount" type="number" />
          <TextInput register={register} errors={errors} label="Para Birimi" name="currency" />
          <TextInput register={register} errors={errors} label="Bitiş Tarihi" name="reservedUntil" type="datetime-local" />
        </div>
      ...
      <fieldset className="border rounded-lg p-4 space-y-4">
        ...
        <TextArea register={register} errors={errors} label="Notlar" name="notes" helperText="Rezervasyon notu." />
        <TextArea register={register} errors={errors} label="İptal Sebebi" name="cancelReason" helperText="İptal edilen rezervasyonlar için." />
      </fieldset>
