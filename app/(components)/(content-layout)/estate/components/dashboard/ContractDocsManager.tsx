...
import { addContractDocument, deleteContractDocument } from "../../actions/contract-documents";
...
export default function ContractDocsManager({ contractId, initialDocs }: Props) {
  const fileInputRef               = useRef<HTMLInputElement>(null);
  const [docs, setDocs]            = useState<Doc[]>(initialDocs);
  const [uploading, setUploading]  = useState(false);
  const [pending, startTransition] = useTransition();
  const [selType, setSelType]      = useState("CONTRACT");
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const saved = await addContractDocument({
        contractId,
        type:  selType,
        title: title.trim(),
        url:   data.secure_url,
  ...
  function handleDelete(docId: string) {
    startTransition(async () => {
      try {
        await deleteContractDocument(docId, contractId);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="space-y-1 sm:col-span-2">
            ...
            <Input
              ...
              placeholder="Örn. Kira Sözleşmesi 2025, Tapu Senedi"
