...
import { addAgentDocument, deleteAgentDocument } from "../../actions/agent-documents";
...
export default function AgentDocsManager({ agentId, initialDocs }: Props) {
  const fileInputRef               = useRef<HTMLInputElement>(null);
  const [docs, setDocs]            = useState<Doc[]>(initialDocs);
  const [uploading, setUploading]  = useState(false);
  const [pending, startTransition] = useTransition();
  const [selType, setSelType]      = useState("ID_PROOF");
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const saved = await addAgentDocument({
        agentId,
        type:  selType,
        title: title.trim(),
        url:   data.secure_url,
  ...
  function handleDelete(docId: string) {
    startTransition(async () => {
      try {
        await deleteAgentDocument(docId, agentId);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div className="space-y-1 sm:col-span-2">
            ...
            <Input
              ...
              placeholder="Örn. Emlak Danışmanlığı Belgesi, Kimlik"
